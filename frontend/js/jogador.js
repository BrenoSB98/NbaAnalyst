...
async function carregarJogador() {
  try {
    ...
    document.getElementById("jogador-avatar").textContent =
    ...
    if (timeAtualObj) {
      infoHtml =
        ...
        '<a href="time.html?id=' +
        timeAtualObj.time_id +
        '" style="color:#555570;text-decoration:none;">' +
        timeAtualObj.nome_time +
        "</a>" +
        "</span>";
    }
    if (dados.inicio_nba) {
      infoHtml =
        infoHtml +
        '<span><i class="bi bi-calendar2"></i>NBA desde ' +
        dados.inicio_nba +
        "</span>";
    }
    if (dados.pais_nascimento) {
      infoHtml =
        infoHtml +
        '<span><i class="bi bi-globe2"></i>' +
        dados.pais_nascimento +
        "</span>";
    }
    document.getElementById("jogador-info-basica").innerHTML = infoHtml;
    document.getElementById("btn-perfil-avancado").href =
      "jogador_avancado.html?id=" + jogadorId;
...
function renderizarDadosFisicos(dados) {
  ...
  if (dados.altura_metros) {
    itens.push({
      icone: "bi-arrow-up",
      valor: dados.altura_metros.toFixed(2) + " m",
      rotulo: "Altura",
    });
  }
  if (dados.peso_kg) {
    itens.push({
      ...
      valor: dados.peso_kg.toFixed(1) + " kg",
      rotulo: "Peso",
    });
  }
  ...
  if (dados.inicio_nba) {
    itens.push({
      icone: "bi-trophy",
      valor: dados.inicio_nba,
      rotulo: "Entrada na NBA",
    });
  }

  if (itens.length === 0) {
    container.innerHTML =
      '<div class="col-12"><p class="texto-suave" style="font-size:0.88rem;">Dados físicos não disponíveis.</p></div>';
    return;
  }
...
function renderizarHistorico(historico) {
  ...
  for (var i = 0; i < historico.length; i++) {
    ...
    var camisa =
      item.camisa !== null && item.camisa !== undefined
        ? "#" + item.camisa
        : "";
    ...
    if (item.mes_ingresso) {
      periodoHtml =
        '<div class="time-temporada">' + item.mes_ingresso + "</div>";
    } else {
    ...
    html =
      ...
      '<a href="time.html?id=' +
      item.time_id +
      '" class="time-nome-link">' +
      item.nome_time +
      "</a>" +
      '<div class="time-posicao">' +
      posicao +
      "</div>" +
      '<div class="time-camisa">' +
      camisa +
      "</div>" +
      "</div>";
  }
...
async function carregarMedias() {
  try {
    ...
    for (var ai = 0; ai < anos.length; ai++) {
      var resp = await chamarApi(
        "/jogadores/" +
          jogadorId +
          "/estatisticas/temporada?temporada=" +
          anos[ai],
      );
    ...
    for (var i = 0; i < itens.length; i++) {
      ...
      html =
        ...
        '<div class="card-media" style="border-top:3px solid ' +
        item.cor +
        ';">' +
        '<div class="media-valor" style="color:' +
        item.cor +
        ';">' +
        valorExibido +
        "</div>" +
        '<div class="media-rotulo">' +
        item.rotulo +
        "</div>" +
        "</div>" +
        "</div>";

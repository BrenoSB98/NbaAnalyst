...
async function carregarTimes() {
  try {
    ...
    var opcaoErroCopia = opcaoErro.cloneNode(true);

    selectCasa.innerHTML = "";
    selectCasa.appendChild(opcaoErro);
    selectFora.innerHTML = "";
    selectFora.appendChild(opcaoErroCopia);

    if (btnBuscar) {
      btnBuscar.disabled = true;
      btnBuscar.title = "Não foi possível carregar a lista de times";
    }
  }
}
...
async function buscarConfrontos() {
  ...
  if (!timeCasaId || !timeForaId) {
    document.getElementById("msg-erro").textContent =
      "Selecione os dois times antes de analisar.";
    esconderTudo();
  ...
  try {
    ...
    esconderTudo();
    document.getElementById("msg-erro").textContent =
      "Não foi possível carregar os dados do confronto.";
    document.getElementById("pagina-erro").style.display = "block";
  }
}
...
function renderizarCardTime(elementId, infoTime, corBorda) {
  ...
  if (infoTime.medias) {
    jogosTexto =
      '<div class="time-info-jogos">Média dos últimos ' +
      infoTime.medias.jogos_considerados +
      " jogos</div>";
  }
  ...
  container.innerHTML =
    logoHtml +
    '<div class="time-info">' +
    '<div class="time-info-nome">' +
    infoTime.nome +
    "</div>" +
    '<div class="time-info-apelido">' +
    (infoTime.apelido || "") +
    "</div>" +
    jogosTexto +
    "</div>";

...
async function carregarKpis() {
    try {
        ...
        document.getElementById("kpi-modelos").textContent = info.total_modelos || 0;
    } catch (e) {
    ...
    try {
        ...
        document.getElementById("kpi-winrate").textContent = Math.round(wr.win_rate_geral || 0) + "%";
    } catch (e) {
    ...
    try {
        ...
        document.getElementById("kpi-relatorios").textContent = relatorios.total || 0;
    } catch (e) {
...
async function carregarRelatorios() {
    ...
    try {
        ...
        for (var i = 0; i < dados.relatorios.length; i++) {
            ...
            html = html + "<div class=\"relatorio-icone\"><i class=\"bi bi-file-earmark-pdf-fill\"></i></div>";
            ...
            html = html + "</div>";
            html = html + "<div class=\"relatorio-meta\">" + dataFormatada + " &nbsp;·&nbsp; " + tamanho + "</div>";
            html = html + "</div>";
            html = html + "<a href=\"#\" class=\"btn-baixar\" onclick=\"baixarRelatorio('" + nome + "', event)\">";
...
async function retreinarModelos() {
    ...
    try {
        ...
        resultado.innerHTML = "<i class=\"bi bi-clock-history me-2\"></i>Retreinamento iniciado. O relatório PDF será gerado ao final e aparecerá na lista abaixo. Isso pode levar alguns minutos.";

        iniciarPollingRelatorios();
    } catch (erro) {
        resultado.style.display = "block";
        resultado.className = "acao-resultado acao-erro";
        resultado.innerHTML = "<i class=\"bi bi-x-circle-fill me-2\"></i>Erro ao iniciar o retreinamento.";
    }

    btn.disabled = false;
    btn.innerHTML = "<i class=\"bi bi-arrow-repeat me-2\"></i>Iniciar Retreinamento";
}
...
async function executarRetroativo() {
    ...
    try {
        ...
        resultado.style.display = "block";
        resultado.className = "acao-resultado acao-sucesso";
        resultado.innerHTML = "<i class=\"bi bi-check-circle-fill me-2\"></i>Predições retroativas geradas: " + (dados.total || 0) + " palpites.";
    } catch (erro) {
        resultado.style.display = "block";
        resultado.className = "acao-resultado acao-erro";
        resultado.innerHTML = "<i class=\"bi bi-x-circle-fill me-2\"></i>Erro ao gerar predições retroativas.";
    }

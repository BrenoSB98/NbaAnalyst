...
def carregar_stats_times_jogo(game_id):
    ...
    for db in get_db():
        ...
        for item in dados_stats:
            ...
            time_existe = db.execute(select(Team).where(Team.id == id_time)).scalar_one_or_none()
            if not time_existe:
                continue
            ...
            stats_existente = db.execute(select(GameTeamStats).where(GameTeamStats.game_id == game_id, GameTeamStats.team_id == id_time)).scalar_one_or_none()
            ...
            nova_stat = GameTeamStats(game_id=game_id, team_id=id_time, fast_break_points=_normalizar_inteiro(estatisticas.get("fastBreakPoints")), points_in_paint=_normalizar_inteiro(estatisticas.get("pointsInPaint")), biggest_lead=_normalizar_inteiro(estatisticas.get("biggestLead")), second_chance_points=_normalizar_inteiro(estatisticas.get("secondChancePoints")), points_off_turnovers=_normalizar_inteiro(estatisticas.get("pointsOffTurnovers")), longest_run=_normalizar_inteiro(estatisticas.get("longestRun")), points=_normalizar_inteiro(estatisticas.get("points")), fgm=_normalizar_inteiro(estatisticas.get("fgm")), fga=_normalizar_inteiro(estatisticas.get("fga")), fgp=_normalizar_decimal(estatisticas.get("fgp")), ftm=_normalizar_inteiro(estatisticas.get("ftm")), fta=_normalizar_inteiro(estatisticas.get("fta")), ftp=_normalizar_decimal(estatisticas.get("ftp")), tpm=_normalizar_inteiro(estatisticas.get("tpm")), tpa=_normalizar_inteiro(estatisticas.get("tpa")), tpp=_normalizar_decimal(estatisticas.get("tpp")), off_reb=_normalizar_inteiro(estatisticas.get("offReb")), def_reb=_normalizar_inteiro(estatisticas.get("defReb")), tot_reb=_normalizar_inteiro(estatisticas.get("totReb")), assists=_normalizar_inteiro(estatisticas.get("assists")), p_fouls=_normalizar_inteiro(estatisticas.get("pFouls")), steals=_normalizar_inteiro(estatisticas.get("steals")), turnovers=_normalizar_inteiro(estatisticas.get("turnovers")), blocks=_normalizar_inteiro(estatisticas.get("blocks")), plus_minus=_normalizar_inteiro(estatisticas.get("plusMinus")), minutes=_normalizar_string(estatisticas.get("min")))
            db.add(nova_stat)
...
def carregar_stats_todos_times(season, team_id=None, data=None):
    ...
    for db in get_db():
        ...
        for idx, jogo in enumerate(jogos, start=1):
            ...
            if idx % 50 == 0:
                logger.info(f"Progresso: {idx}/{total_jogos} ({round(idx/total_jogos*100)}%).")
...
def carregar_stats_temporada_time(team_id, season):
    ...
    for db in get_db():
        ...
        stats_existente = db.execute(select(TeamSeasonStats).where(TeamSeasonStats.team_id == team_id, TeamSeasonStats.season == season)).scalar_one_or_none()
        ...
        if stats_existente:
            ...
            nova_stat = TeamSeasonStats(team_id=team_id, season=season, games=jogos_total, points=pontos_total, fgm=fgm, fga=fga, fgp=fgp, ftm=ftm, fta=fta, ftp=ftp, tpm=tpm, tpa=tpa, tpp=tpp, off_reb=off_reb, def_reb=def_reb, tot_reb=tot_reb, assists=assists, p_fouls=p_fouls, steals=steals, turnovers=turnovers, blocks=blocks, plus_minus=plus_minus, fast_break_points=fast_break_points, points_in_paint=points_in_paint, biggest_lead=biggest_lead, second_chance_points=second_chance_points, points_off_turnovers=points_off_turnovers, longest_run=longest_run)
            db.add(nova_stat)
...
def carregar_stats_temporada_todos_times(season):
    ...
    for db in get_db():
        times = db.execute(select(Team).where(Team.nba_franchise == True)).scalars().all()
        ...
        for time_item in times:
            try:
                carregar_stats_temporada_time(team_id=time_item.id, season=season)
                total_ok = total_ok + 1
            except Exception as erro:
                logger.warning(f"Erro time={time_item.id}: {erro}")
                continue

...
def listar_jogos(temporada: int = Query(None), time_id: int = Query(None), data_inicio: str = Query(None), data_fim: str = Query(None), status: int = Query(None), page: int = Query(1, ge=1), page_size: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    stmt = select(Game)

    if temporada is not None:
        stmt = stmt.where(Game.season == temporada)
    if time_id is not None:
        stmt = stmt.where(or_(Game.home_team_id == time_id, Game.away_team_id == time_id))
    if data_inicio:
        stmt = stmt.where(Game.date_start >= data_inicio)
    if data_fim:
        stmt = stmt.where(Game.date_start <= data_fim)
    if status is not None:
        stmt = stmt.where(Game.status_short == status)

    total = db.execute(select(func.count()).select_from(stmt.subquery())).scalar()
    offset = (page - 1) * page_size
    jogos = db.execute(stmt.order_by(Game.date_start.desc()).offset(offset).limit(page_size)).scalars().all()
...
def contar_jogos_hoje(db: Session = Depends(get_db)):
    ...
    total = db.execute(select(func.count(Game.id)).where(Game.date_start >= inicio_utc, Game.date_start < fim_utc)).scalar()
...
def proximos_jogos(dias: int = Query(7, ge=1, le=30), time_id: int = Query(None), db: Session = Depends(get_db)):
    ...
    stmt = select(Game).where(Game.date_start >= hoje, Game.date_start <= data_limite, Game.status_short == 1)

    if time_id:
        stmt = stmt.where(or_(Game.home_team_id == time_id, Game.away_team_id == time_id))

    jogos = db.execute(stmt.order_by(Game.date_start.asc())).scalars().all()

    lista_jogos = []
    for jogo in jogos:
        time_casa = db.execute(select(Team).where(Team.id == jogo.home_team_id)).scalar_one_or_none()
        time_fora = db.execute(select(Team).where(Team.id == jogo.away_team_id)).scalar_one_or_none()
...
def obter_jogo(jogo_id: int, db: Session = Depends(get_db)):
    jogo = db.execute(select(Game).where(Game.id == jogo_id)).scalar_one_or_none()
    if not jogo:
        logger.warning(f"Jogo nao encontrado: id={jogo_id}")
        raise HTTPException(status_code=404, detail="Jogo não encontrado.")

    scores = db.execute(select(GameTeamScore).where(GameTeamScore.game_id == jogo_id)).scalars().all()
    ...
    return {
        "id": jogo.id, "temporada": jogo.season, "liga": jogo.league, "data_inicio": jogo.date_start, "data_fim": jogo.date_end,
        "duracao": jogo.duration, "status_short": jogo.status_short, "status_long": jogo.status_long, "fase": jogo.stage,
        "periodos_atual": jogo.periods_current, "periodos_total": jogo.periods_total,
        "arena": {"nome": jogo.arena_name, "cidade": jogo.arena_city, "estado": jogo.arena_state, "pais": jogo.arena_country},
        "time_casa": info_casa, "time_fora": info_fora, "empates": jogo.times_tied,
        "mudancas_lideranca": jogo.lead_changes, "nugget": jogo.nugget,
    }
...
def estatisticas_times_jogo(jogo_id: int, db: Session = Depends(get_db)):
    jogo = db.execute(select(Game).where(Game.id == jogo_id)).scalar_one_or_none()
    if not jogo:
        logger.warning(f"Jogo nao encontrado ao buscar estatisticas de times: id={jogo_id}")
        raise HTTPException(status_code=404, detail="Jogo não encontrado.")

    stats = db.execute(select(GameTeamStats).where(GameTeamStats.game_id == jogo_id)).scalars().all()

    lista_stats = []
    for stat in stats:
        time = db.execute(select(Team).where(Team.id == stat.team_id)).scalar_one_or_none()
        if time:
            nome_time = time.name
        else:
            nome_time = None
        ...
        lista_stats.append({
            "time_id": stat.team_id, "nome_time": nome_time, "pontos": stat.points,
            ...
            "rebotes_ofensivos": stat.off_reb, "rebotes_defensivos": stat.def_reb, "rebotes_totais": stat.tot_reb,
            "assistencias": stat.assists, "roubos": stat.steals, "bloqueios": stat.blocks, "turnovers": stat.turnovers,
            "faltas": stat.p_fouls, "plus_minus": stat.plus_minus, "fast_break_points": stat.fast_break_points,
            "pontos_no_garrafao": stat.points_in_paint, "segundas_chances": stat.second_chance_points,
            "pontos_apos_turnover": stat.points_off_turnovers, "maior_vantagem": stat.biggest_lead, "maior_sequencia": stat.longest_run,
        })
...
def estatisticas_jogadores_jogo(jogo_id: int, db: Session = Depends(get_db)):
    jogo = db.execute(select(Game).where(Game.id == jogo_id)).scalar_one_or_none()
    if not jogo:
        logger.warning(f"Jogo nao encontrado ao buscar estatisticas de jogadores: id={jogo_id}")
        raise HTTPException(status_code=404, detail="Jogo não encontrado.")

    stmt = select(PlayerGameStats, Player).join(Player, PlayerGameStats.player_id == Player.id).where(PlayerGameStats.game_id == jogo_id)
    ...
    for stat_data in stats:
        ...
        lista_stats.append({
            "jogador_id": stat.player_id, "nome_jogador": f"{jogador.firstname} {jogador.lastname}",
            "time_id": stat.team_id, "posicao": stat.pos, "minutos": stat.minutes, "pontos": stat.points,
            "assistencias": stat.assists, "rebotes_totais": stat.tot_reb, "rebotes_ofensivos": stat.off_reb,
            "rebotes_defensivos": stat.def_reb, "roubos": stat.steals, "bloqueios": stat.blocks,
            "turnovers": stat.turnovers, "faltas": stat.p_fouls,
            "fgm": stat.fgm, "fga": stat.fga, "fgp": float(stat.fgp) if stat.fgp is not None else None,
            "ftm": stat.ftm, "fta": stat.fta, "ftp": float(stat.ftp) if stat.ftp is not None else None,
            "tpm": stat.tpm, "tpa": stat.tpa, "tpp": float(stat.tpp) if stat.tpp is not None else None,
            "plus_minus": stat.plus_minus, "comentario": stat.comment,
        })

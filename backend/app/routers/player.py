...
def listar_jogadores(time_id: int = Query(None), temporada: int = Query(None), nome: str = Query(None), sobrenome: str = Query(None), page: int = Query(1, ge=1), page_size: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    stmt = select(Player)
    ...
    jogadores = db.execute(stmt.order_by(Player.lastname.asc()).offset(offset).limit(page_size)).scalars().all()

    lista_jogadores = []
    for jogador in jogadores:
        lista_jogadores.append({
            "id": jogador.id, "nome": jogador.firstname, "sobrenome": jogador.lastname,
            "data_nascimento": jogador.birth_date, "pais_nascimento": jogador.birth_country,
...
def obter_jogador(jogador_id: int, db: Session = Depends(get_db)):
    jogador = db.execute(select(Player).where(Player.id == jogador_id)).scalar_one_or_none()
    if not jogador:
        logger.warning(f"Jogador nao encontrado: id={jogador_id}")
        raise HTTPException(status_code=404, detail="Jogador não encontrado.")

    stmt = select(PlayerTeamSeason, Team).join(Team, PlayerTeamSeason.team_id == Team.id).where(PlayerTeamSeason.player_id == jogador_id)
    ...
    for time_data in times_jogador:
        ...
        stmt_jogo = (select(Game.date_start)
                     .join(PlayerGameStats, PlayerGameStats.game_id == Game.id)
                     .where(PlayerGameStats.player_id == jogador_id, PlayerGameStats.team_id == pts.team_id, Game.season == pts.season)
        ...
        historico_times_raw.append({
            "temporada": pts.season, "time_id": pts.team_id, "nome_time": time.name,
            "camisa": pts.jersey, "posicao": pts.pos, "ativo": pts.active,
            "mes_ingresso": mes_ingresso, "_data_sort": data_ingresso,
        })
    ...
    for item in historico_times_raw:
        historico_times.append({
            "temporada": item["temporada"], "time_id": item["time_id"], "nome_time": item["nome_time"],
            "camisa": item["camisa"], "posicao": item["posicao"], "ativo": item["ativo"], "mes_ingresso": item["mes_ingresso"],
        })

    return {
        "id": jogador.id, "nome": jogador.firstname, "sobrenome": jogador.lastname,
        "data_nascimento": jogador.birth_date, "pais_nascimento": jogador.birth_country,
        "inicio_nba": jogador.nba_start, "altura_pes": jogador.height_feet, "altura_polegadas": jogador.height_inches,
        "altura_metros": float(jogador.height_meters) if jogador.height_meters is not None else None,
        "peso_libras": jogador.weight_pounds,
        "peso_kg": float(jogador.weight_kilograms) if jogador.weight_kilograms is not None else None,
        "faculdade": jogador.college, "afiliacao": jogador.affiliation, "historico_times": historico_times,
    }
...
def estatisticas_temporada_jogador(jogador_id: int, temporada: int = Query(2025), db: Session = Depends(get_db)):
    jogador = db.execute(select(Player).where(Player.id == jogador_id)).scalar_one_or_none()
    if not jogador:
        logger.warning(f"Jogador nao encontrado ao buscar estatisticas de temporada: id={jogador_id}")
        raise HTTPException(status_code=404, detail="Jogador não encontrado.")

    stmt = select(PlayerGameStats).join(Game, PlayerGameStats.game_id == Game.id).where(PlayerGameStats.player_id == jogador_id, Game.season == temporada, Game.status_short == 3)
    ...
    return {
        "jogador_id": jogador_id, "nome_jogador": f"{jogador.firstname} {jogador.lastname}",
        "temporada": temporada, "jogos_disputados": num_jogos,
        ...
        "medias": {
            "pontos": round(total_pontos / num_jogos, 2), "assistencias": round(total_assistencias / num_jogos, 2),
            "rebotes": round(total_rebotes / num_jogos, 2), "roubos": round(total_roubos / num_jogos, 2),
            "bloqueios": round(total_bloqueios / num_jogos, 2), "turnovers": round(total_turnovers / num_jogos, 2),
            "plus_minus": round(total_plus_minus / num_jogos, 2), "fg_pct": media_fgp, "three_pct": media_tpp, "ft_pct": media_ftp,
        },
    }
...
def estatisticas_jogos_jogador(jogador_id: int, temporada: int = Query(None), limite: int = Query(10, ge=1, le=100), db: Session = Depends(get_db), usuario_atual=Depends(obter_usuario_atual)):
    jogador = db.execute(select(Player).where(Player.id == jogador_id)).scalar_one_or_none()
    if not jogador:
        logger.warning(f"Jogador nao encontrado ao buscar historico de jogos: id={jogador_id}")
        raise HTTPException(status_code=404, detail="Jogador não encontrado.")

    stmt = select(PlayerGameStats, Game).join(Game, PlayerGameStats.game_id == Game.id).where(PlayerGameStats.player_id == jogador_id)
    ...
    for stat_data in stats:
        ...
        lista_jogos.append({
            "jogo_id": stat.game_id, "data": jogo.date_start, "temporada": jogo.season,
            "adversario_id": adversario_id, "minutos": stat.minutes, "pontos": stat.points,
            "assistencias": stat.assists, "rebotes": stat.tot_reb, "roubos": stat.steals,
            "bloqueios": stat.blocks, "turnovers": stat.turnovers,
...
def estatisticas_ultimos_n_jogos(jogador_id: int, n_jogos: int = Query(None, description="Limite de jogos. Omitir = todos os jogos"), temporada: int = Query(None), db: Session = Depends(get_db)):
    jogador = db.execute(select(Player).where(Player.id == jogador_id)).scalar_one_or_none()
    if not jogador:
        logger.warning(f"Jogador nao encontrado ao buscar ultimos jogos: id={jogador_id}")
        raise HTTPException(status_code=404, detail="Jogador não encontrado.")
    ...
    if len(stats) == 0:
        return {"jogador_id": jogador_id, "nome_jogador": f"{jogador.firstname} {jogador.lastname}", "n_jogos": n_jogos or 0, "jogos_analisados": 0, "temporada": temporada, "medias": None, "jogos": [], "mensagem": "Sem dados para calcular."}
    ...
    for stat_data in stats:
        ...
        lista_jogos.append({
            ...
            "adversario": nome_adversario, "em_casa": em_casa, "pontos": stat.points,
            "assistencias": stat.assists, "rebotes": stat.tot_reb, "roubos": stat.steals,
            "bloqueios": stat.blocks, "turnovers": stat.turnovers, "minutos": stat.minutes,
            "fg_pct": fg_pct_jogo, "three_pct": three_pct_jogo, "ft_pct": ft_pct_jogo, "plus_minus": stat.plus_minus,
        })
    ...
    return {
        "jogador_id": jogador_id, "nome_jogador": f"{jogador.firstname} {jogador.lastname}",
        "n_jogos": n_jogos or 0, "jogos_analisados": num_jogos, "temporada": temporada,
        "medias": {
            "pontos": round(total_pontos / num_jogos, 2), "assistencias": round(total_assistencias / num_jogos, 2),
            "rebotes": round(total_rebotes / num_jogos, 2), "roubos": round(total_roubos / num_jogos, 2),
            "bloqueios": round(total_bloqueios / num_jogos, 2), "turnovers": round(total_turnovers / num_jogos, 2),
...
def estatisticas_casa_fora(jogador_id: int, temporada: int = Query(None), local: str = Query(..., description="casa ou fora"), db: Session = Depends(get_db)):
    jogador = db.execute(select(Player).where(Player.id == jogador_id)).scalar_one_or_none()
    if not jogador:
        logger.warning(f"Jogador nao encontrado ao buscar stats casa/fora: id={jogador_id}")
        raise HTTPException(status_code=404, detail="Jogador não encontrado.")
    ...
    stmt = (select(PlayerGameStats, Game, GameTeamScore)
            ...
            .where(PlayerGameStats.player_id == jogador_id, Game.season == temporada, Game.status_short == 3, GameTeamScore.is_home == buscar_casa))
    ...
    for stat_data in stats_query:
        ...
        lista_jogos.append({
            "jogo_id": jogo.id, "data": jogo.date_start, "pontos": stat.points,
            "assistencias": stat.assists, "rebotes": stat.tot_reb, "roubos": stat.steals,
            "bloqueios": stat.blocks, "turnovers": stat.turnovers,
        })
    ...
    return {
        "jogador_id": jogador_id, "nome_jogador": f"{jogador.firstname} {jogador.lastname}",
        "temporada": temporada, "local": local, "jogos_disputados": num_jogos,
        "medias": {
            "pontos": round(total_pontos / num_jogos, 2), "assistencias": round(total_assistencias / num_jogos, 2),
            "rebotes": round(total_rebotes / num_jogos, 2), "roubos": round(total_roubos / num_jogos, 2),
            "bloqueios": round(total_bloqueios / num_jogos, 2), "turnovers": round(total_turnovers / num_jogos, 2),

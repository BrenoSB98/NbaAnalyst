...
MARGEM_PONTOS = config.MARGEM_PONTOS
MARGEM_ASSISTENCIAS = config.MARGEM_ASSISTENCIAS
MARGEM_REBOTES = config.MARGEM_REBOTES
MARGEM_ROUBOS = config.MARGEM_ROUBOS
MARGEM_BLOQUEIOS = config.MARGEM_BLOQUEIOS
...
def calcular_win_rate(db, temporada):
    palpites_com_real = (db.query(Prediction, PlayerGameStats).join(PlayerGameStats, (PlayerGameStats.player_id == Prediction.player_id) & (PlayerGameStats.game_id == Prediction.game_id))
                          .join(Game, Game.id == Prediction.game_id).filter(Prediction.season == temporada, Game.status_short == 3).all())
    ...
    resultado["total_predicoes_avaliadas"] = len(palpites_com_real)
